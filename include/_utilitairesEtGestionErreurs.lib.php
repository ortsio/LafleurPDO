...
/** 
 * Ajoute un message dans un tableau des messages                 
 * 
 * Ajoute le message $msg en fin de tableau $tab. Ce tableau est passé par 
 * référence afin que les modifications sur ce tableau soient visibles de l'appelant.  
 * @param array $tab
 * @param string message
 * @return void
 */ 
function ajouterMessage(&$tab,$msg) {
    $tab[count($tab)]=$msg;
}

/** 
 * Retourne le nombre de messages enregistrés.                    
 * 
 * Retourne le nombre de messages enregistrés dans le tableau $tab. 
 * @param array $tab tableau des messages
 * @return int nombre de messages
 */ 
function nbMessages($tab) {
    return count($tab);
}
 
/** 
 * Fournit les messages (d'erreurs ou de succes)) sous forme d'une liste à puces HTML.                    
 * 
 * Retourne le source HTML, division contenant une liste à puces, d'après les
 * messages contenus dans le tableau des messages 
 * @param array $tab tableau des messages
 * @return string source html
 */ 
function toStringMessage($tab) {
    $str = '<ul>';
